...
export function WeeklyReviewNextPlanCard({ plan }: { plan: WeeklyReviewNextWeekPlan }) {
  if (!plan || (!plan.focus && !plan.commitments?.length && !plan.risks?.length && !plan.recoveryActions?.length)) {
    return (
      <Card>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {plan.recoveryActions?.length > 0 && (
          <div className="space-y-1.5">
            <div className="text-xs font-medium text-muted-foreground">Recovery Actions</div>
